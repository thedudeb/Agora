...
Workspace Settings include theme presets, density controls, deployment readiness, a current-access summary, company-scoped member access, and a role permission matrix so self-hosters can tune Agora before inviting a team.
...
When connected to the API, Agora loads projects and tasks from `/api/projects` and `/api/tasks`, adopts server-returned records after project/task writes, polls for workspace and structured-record changes, merges server-canonical records back into the browser, shows live task viewers and same-page cursor presence, generates inbox items for mentions and watched tasks, and warns before saving over a task that changed while its modal was open.
