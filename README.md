...
When connected to the API, Agora polls for workspace and structured-record changes, merges server-canonical records back into the browser, shows live task viewers and same-page cursor presence, generates inbox items for mentions and watched tasks, and warns before saving over a task that changed while its modal was open.
