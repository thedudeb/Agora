...
The prototype stores changes in browser local storage. Use the sidebar workspace switcher to create, duplicate, archive, and switch local workspaces. Use the Dashboard setup panel to choose demo data or start with a clean workspace. Use Data to create local workspace backups, download JSON exports, download a portable workspace bundle with JSON/CSV/Markdown/operator context, replace the current workspace from JSON, or import JSON as a new workspace. Use "Reset sample data" in the sidebar to restore the seeded workspace.
