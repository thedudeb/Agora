...
Agora currently covers one primary loop: launch a client workspace, plan work, move tasks, triage PM signals, review client-safe status, recover risky work with human-approved Autopilot proposals, and leave with a portable workspace bundle.

## What You Can Run Today

- Core beta app: dependency-free browser UI with seeded Acme demo data, first-run setup, projects, tasks, board/list/calendar/timeline views, comments, files, reports, client portal, inbox, Project Memory, Project Autopilot previews, local persistence, backups, imports, portable exports, and an offline PWA shell.
- Team and self-hosting path: optional API with local JSON storage, Supabase storage/auth, invitations, roles, permissions, structured records, file uploads, scheduler jobs, production readiness checks, and smoke tests.
- Power-user beta surfaces: CLI, MCP server, desktop shell, migration concierge, release cockpit, Trust Center, plugin contracts, automation packs, template marketplace, sprint command center, portfolio views, and capacity planning.
- Experimental foundations: payment-provider skeletons, marketplace entitlements, native mobile wrapper planning, deeper integrations, and advanced automation remain proof-oriented until release evidence promotes them.
- Trust posture: no ads, no trackers, server-only secrets, company-scoped access, portable bundles, auditable AI actions, and preview/undo paths for high-impact automation.

## Status

Agora is in focused beta hardening. The product is feature-rich enough; current work should improve coherence, release proof, demo clarity, migration safety, trust evidence, and tester feedback loops before adding new product surface area.

The active release target is `v0.1-beta`: guided evaluation by agencies, consultants, self-hosters, and power users using the Acme client-delivery workflow. Native mobile apps, broad public desktop distribution, live payment providers, and deep two-way SaaS integrations require separate release evidence before they are treated as shipped product.
...
Advanced surfaces such as sprint forecasting, portfolio planning, marketplace packs, payments, plugins, MCP, and automation authoring are available for power-user evaluation, but they should be judged as beta/platform proof rather than the first-run promise.
...
npm run distribution:evidence -- --release v0.1-beta # generate a per-channel proof bundle
...
npm run release:check # validate the live release-candidate handoff
...
For beta launch copy, use [`docs/beta-invite-pack.md`](./docs/beta-invite-pack.md) and [`docs/release-notes-v0.1-beta.md`](./docs/release-notes-v0.1-beta.md).
...
For Project Autopilot details, see [`docs/project-autopilot.md`](./docs/project-autopilot.md). It documents the Safety Center, human approval model, one-click undo, Project Memory bridge, first-run demo path, and current limits.
...
2. Run `server/migrations/001_supabase_storage.sql`, `server/migrations/002_supabase_auth_rls.sql`, `server/migrations/003_background_jobs.sql`, and `server/migrations/004_auth_sessions.sql` in the Supabase SQL editor.
...
`002_supabase_auth_rls.sql` adds `public.agora_workspace_memberships`, helper functions around `auth.uid()`, and RLS policies for snapshots, audit events, and structured record tables. `003_background_jobs.sql` persists retryable email and worker job state. `004_auth_sessions.sql` persists hashed API session ids for durable rotation, revocation, and account-security visibility. The API can sign users up or in with Supabase email/password auth from Settings, exchange a Supabase Auth `access_token`, or accept that token directly as a Bearer token when `AGORA_AUTH_DRIVER=supabase`.
...
Project Autopilot is the PM-facing recovery layer on top of project data and Project Memory. It detects schedule, blocker, approval, workload, scope, and client-promise drift; shows a Safety Center before proposals; simulates downstream impact; and applies scoped recovery moves only after human approval. Applied scenarios write audit events with undo snapshots so changed tasks can be restored and Autopilot-created tasks or RAID items can be removed from the Safety Center.
