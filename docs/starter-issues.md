...
Use [`checks.md`](./checks.md) to pick the smallest verification gate for your change. Every starter issue below includes at least one focused command so contributors can prove the work without guessing.
...
- Run `npm run check`.
- Run `npm run test:golden:mobile` if the fix changes mobile route behavior.
